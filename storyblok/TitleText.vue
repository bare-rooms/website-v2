...
<template>
  <div v-editable="blok" :style="{ backgroundColor: blok.background_color?.color }" :class="sectionClass">
    <h2 v-if="!!blok?.title" class="text-secondary font-heading my-2" :style="{color: blok.title_color?.color}"
      :class="blok?.text_center ? 'text-center' : 'text-left'">
        {{ blok?.title }}
    </h2>
    <h4 class="font-heading my-2" :style="{color: blok.subtitle_color?.color}" :class="blok?.text_center ? 'text-center' : 'text-left'">
      {{ blok?.subtitle }}
    </h4>
